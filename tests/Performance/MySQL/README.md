# MySQL Performance Tests for RapidBase

Este directorio contiene pruebas de rendimiento para evaluar el desempeño de **RapidBase** trabajando con **MySQL** como motor de base de datos.
...
- **UPSERT (ON DUPLICATE KEY)**: Inserción con actualización en conflicto

### 8. Consultas Complejas
- **Query compleja (reporte de ventas)**: Reporte con múltiples JOINs, agregaciones y filtros
- **Window Function (RANK)**: Funciones de ventana para ranking (MySQL 8.0+)

## 🚀 Ejecución

```bash
php tests/Performance/MySQL/MySQLPerformanceTest.php
```
...
```sql
-- Crear base de datos
CREATE DATABASE rapidbase_test CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Crear usuario
CREATE USER 'rapidbase_user'@'localhost' IDENTIFIED BY 'rapidbase_pass';

-- Otorgar permisos
GRANT ALL PRIVILEGES ON rapidbase_test.* TO 'rapidbase_user'@'localhost';
FLUSH PRIVILEGES;
```
...
4. **Configuración de MySQL**: innodb_buffer_pool_size, query_cache_size, etc.
...
2. **Query optimization**: Usar EXPLAIN para identificar cuellos de botella
...
Para comparar el rendimiento entre MySQL y PostgreSQL:

1. Ejecutar este test con MySQL
2. Ejecutar test equivalente con PostgreSQL
3. Comparar:
   - Tiempos promedio por tipo de operación
   - Escalabilidad con mayor volumen de datos
   - Características exclusivas (JSON, window functions, CTEs, etc.)
...
- Las funciones de ventana requieren MySQL 8.0 o superior
