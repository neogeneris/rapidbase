...
class MedooAdapter {
    ...
    public function __construct($pdo) {
        $this->db = new Medoo([
            ...
            'username' => 'rapidbase_user',
            'password' => 'rapidbase_pass',
            'port' => 3306,
            'pdo' => $pdo
        ]);
    }
...
class PixieAdapter {
    ...
    public function __construct($pdo) {
        // Pixie crea su propia conexión interna para MySQL
...
class F3Adapter {
    ...
    public function __construct($pdo) {
        ...
        $this->pdo = $pdo;
        $this->db = new \DB\SQL('mysql:host=localhost;port=3306;dbname=rapidbase_test', 'rapidbase_user', 'rapidbase_pass');
...
class QNoCacheAdapter {
    ...
    public function __construct($pdo) {
        ...
        \RapidBase\Core\Conn::setup('mysql:host=localhost;port=3306;dbname=rapidbase_test', 'rapidbase_user', 'rapidbase_pass', $this->connectionId);
...
class RedbeanAdapter {
    ...
    public function __construct($pdo) {
        ...
        // Configurar RedBean para MySQL con DSN explícito
        \RedBeanPHP\R::setup('mysql:host=localhost;port=3306;dbname=rapidbase_test', 'rapidbase_user', 'rapidbase_pass');
...
echo "Comparing: PDO, Medoo, Pixie, F3 (DB), Q (no cache), Q (cache)\n";
echo str_repeat('=', 80) . "\n\n";
