...
$qAutoload = __DIR__ . '/../../vendor/autoload.php';
...
if (file_exists($f3Autoload)) {
    ...
    require_once __DIR__ . '/F3/vendor/bcosca/fatfree-core/base.php';
    require_once __DIR__ . '/F3/vendor/bcosca/fatfree-core/db/sql.php';
}
    ...
    public function __construct($pdo) {
        ...
        foreach ($tables as $table) {
            ...
            if (!empty($data)) {
                ...
                $stmt = $newPdo->prepare("INSERT INTO $table ($colList) VALUES ($placeholders)");
                foreach ($data as $row) {
...
class F3Adapter {
    ...
    public function __construct($pdo) {
        // F3 necesita una cadena DSN, no un objeto PDO directamente
        $this->pdo = $pdo;
        $this->db = new \DB\SQL('sqlite::memory:');
        // Transferir tablas y datos desde el PDO original
        $tables = $pdo->query("SELECT name FROM sqlite_master WHERE type='table'")->fetchAll(PDO::FETCH_COLUMN);
        foreach ($tables as $table) {
            if ($table === 'sqlite_sequence') continue;
            $schema = $pdo->query("SELECT sql FROM sqlite_master WHERE type='table' AND name='$table'")->fetchColumn();
            ...
            if (!empty($data)) {
                ...
                $placeholders = implode(',', array_fill(0, count($columns), '?'));
                $f3Pdo = $this->db->pdo();
                $stmt = $f3Pdo->prepare("INSERT INTO $table ($colList) VALUES ($placeholders)");
                foreach ($data as $row) {
                    $stmt->execute(array_values($row));
                }
            }
...
class RedbeanAdapter {
    ...
    public function __construct($pdo) {
        ...
        // Definir las constantes antes de cargar para evitar error con MySQL
        if (!defined('RB_PDO_MYSQL_ATTR_INIT_COMMAND')) {
            define('RB_PDO_MYSQL_ATTR_INIT_COMMAND', 1000); // Valor dummy para SQLite
        }
        // Patchear el archivo Functions.php para evitar el error
...
$medooPdo = new PDO('sqlite::memory:');
...
$results[] = ['orm' => 'Medoo', 'test' => 'Delete', ...runBenchmark('Medoo Delete', fn() => $medoo->delete('users', ['id' => 1]))];

// ----------------------------------------------------------------------------
// PIXIE
// ----------------------------------------------------------------------------
echo "Running Pixie benchmarks...\n";
$pixiePdo = new PDO('sqlite::memory:');
...
$results[] = ['orm' => 'Pixie', 'test' => 'Delete', ...runBenchmark('Pixie Delete', fn() => $pixie->delete('users', ['id' => 1]))];

// ----------------------------------------------------------------------------
// F3 (Fat-Free Framework DB)
// ----------------------------------------------------------------------------
echo "Running F3 benchmarks...\n";
$f3Pdo = new PDO('sqlite::memory:');
...
$results[] = ['orm' => 'Q', 'test' => 'Delete', ...runBenchmark('Q Delete', fn() => $q->delete('users', ['id' => 1]))];

// ----------------------------------------------------------------------------
// REDBEAN
// ----------------------------------------------------------------------------
echo "Running Redbean benchmarks...\n";
$redbeanPdo = new PDO('sqlite::memory:');
...
$results[] = ['orm' => 'Redbean', 'test' => 'Delete', ...runBenchmark('Redbean Delete', fn() => $redbean->delete('users', 1))];
