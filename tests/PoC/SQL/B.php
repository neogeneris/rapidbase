...
/**
 * Clase B: Builder - Inicia la cadena de responsabilidad.
 * 
 * Contiene los métodos que inician consultas:
 * - from() y todos sus métodos auxiliares
 * - Métodos de configuración (where, orderBy, limit, etc.)
 * 
 * Esta clase es el inicio del flujo pero NO ejecuta ni finaliza la consulta.
 * Para obtener resultados, debe encadenarse con métodos de la clase F.
 */
class B
{
    ...
    public function where(array $filter): self
    {
        if (!empty($filter)) {
            $whereKey = self::getWhereCacheKey($filter);
            if (isset(self::$whereCache[$whereKey])) {
                $this->state[self::T_WHR] = self::$whereCache[$whereKey]['sql'];
                $this->state[self::T_PRM] = array_merge($this->state[self::T_PRM], self::$whereCache[$whereKey]['params']);
            } else {
                $parsed = self::parseWhere($filter);
                self::$whereCache[$whereKey] = $parsed;
     ...
     */
    public function page(int $currentPage, int $pageSize = self::DEF_PAGE): self
    {
        $offset = ($currentPage - 1) * $pageSize;
        $this->state[self::T_LIM] = $pageSize;
        $this->state[self::T_OFF] = max(0, $offset);
        return $this;
    }
    ...
    {
        $this->state[self::T_GRP] = implode(', ', $columns);
        return $this;
    }
    ...
    /**
     * Obtiene el estado interno para pasar a F.
     * @return array
     */
