...
/**
 * Prueba de JOIN automático con Q (nuevo motor)
 *
 * Casos:
 * - 1:n (users → posts)
 * - n:1 (posts → users)
...
// 2. Definir el mapa de relaciones y cargarlo en SchemaMap
$relationsMap = [
    'relationships' => [
        'from' => [
            'users' => [
...
// Añadir metadatos de columnas para las tablas (requerido para expandir '*')
$fullMap = array_merge($relationsMap, [
    'tables' => [
        'users' => [
            'id' => ['type' => 'int'],
...
// 1:n
echo "\n--- 1:n (users ? posts) ---\n";
$result = Gateway::select('*', ['users', 'posts'], ['users.id' => 1]);
$data = $result['data'];
assertJoin("Número de registros", count($data) === 2);
assertJoin("Columnas esperadas", isset($data[0]['username']) && isset($data[0]['title']));

// n:1
echo "\n--- n:1 (posts ? users) ---\n";
$result = Gateway::select('*', ['posts', 'users'], ['posts.id' => 1]);
$data = $result['data'];
...
// 1:1
echo "\n--- 1:1 (users ? profiles) ---\n";
$result = Gateway::select('*', ['users', 'profiles'], ['users.id' => 2]);
$data = $result['data'];
...
$result = Gateway::select('*', ['posts', 'post_tag', 'tags'], ['posts.id' => 1]);
$data = $result['data'];
...
$result = Gateway::select('*', ['tags', 'post_tag', 'posts'], ['tags.name' => 'Racing']);
$data = $result['data'];
...
$result = Gateway::select('*', ['users', 'posts', 'post_tag', 'tags'], ['users.username' => 'Alice']);
$data = $result['data'];
...
// Añadir categorías al mapa de relaciones y al esquema de tablas
$fullMap['relationships']['from']['categories']['categories'] = [
    'type' => 'belongsTo',
    'local_key' => 'parent_id',
    'foreign_key' => 'id'
];
$fullMap['tables']['categories'] = [
    'id' => ['type' => 'int'],
    'name' => ['type' => 'varchar'],
    'parent_id' => ['type' => 'int']
];
...
// Caso 1: Usar alias manuales en fields
...
// Caso 2: Usar '*' con esquema completo (ahora sí disponible)
$result = Gateway::select('*', ['categories', 'categories as parent'], ['categories.name' => 'Laptops']);
$data = $result['data'];
assertJoin("Auto-referencia con '*' y esquema", isset($data[0]['parent_name']) && $data[0]['parent_name'] === 'Computadoras');
