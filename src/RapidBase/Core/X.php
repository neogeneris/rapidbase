...
class X
{
    ...
    /**
     * Obtiene la descripción (metadata) de las tablas especificadas
     * Incluye columnas, tipos de datos, primary keys y relaciones
     * 
     * @return array Formato: [ 'tabla' => [ 'columns' => [...], 'pks' => [...], 'relations' => [...] ], ... ]
     */
    public function description(): array
    {
        ...
        if ($this->table === null) {
            throw new \RuntimeException("No table(s) selected. Call ->from() before using description().");
        }
        ...
        foreach ($tables as $tableName) {
            ...
            $sql = match($driver) {
                ...
                default => throw new \RuntimeException("Driver $driver not supported for description()")
            };
            ...
            // Obtener PKs para PostgreSQL
            if ($driver === 'pgsql' && empty($pks)) {
            ...
            if ($driver === 'sqlite') {
                $fkSql = "PRAGMA foreign_key_list('$tableName')";
                $fkStmt = $pdo->query($fkSql);
                while ($fkRow = $fkStmt->fetch(\PDO::FETCH_ASSOC)) {
                    $relations[] = [
                        'direction' => 'out',
                        'local_col' => $fkRow['from'],
                        'target_table' => $fkRow['table'],
                        'target_col' => $fkRow['to'],
                        'type' => 'belongsTo'
                    ];
                ...
                while ($fkRow = $fkStmt->fetch(\PDO::FETCH_ASSOC)) {
                    $relations[] = [
                        'direction' => 'out',
                        'local_col' => $fkRow['COLUMN_NAME'],
                        'target_table' => $fkRow['REFERENCED_TABLE_NAME'],
                        'target_col' => $fkRow['REFERENCED_COLUMN_NAME'],
                        'type' => 'belongsTo'
                    ];
                }
            }
