 ...
 *
 * Compatible with:
 *   Old format: { database, schema_tables: [{name, columns:[{name,type,primary,nullable}]}] }
 *   New format: { database, tables: [{name, columns: {col: type}, primaryKeys: [...] }] }
 */
class TableList {
    ...
    setLoading() {
        if (!this.container) return;
        this.container.innerHTML = `<div class="tl-wrapper"><div class="tl-state">Cargando esquema…</div></div>`;
    }

    setError(msg) {
        if (!this.container) return;
        this.container.innerHTML = `<div class="tl-wrapper"><div class="tl-state error">⚠ ${window.escapeHtml(msg)}</div></div>`;
    }
    ...
    _renderEmpty() {
        this.container.innerHTML = `<div class="tl-wrapper"><div class="tl-state">Selecciona una conexión para ver el esquema</div></div>`;
    }
    ...
    _render() {
        ...
        // Obtener lista de tablas (compatible con ambos formatos)
        const rawTables = data.schema_tables || data.tables || [];
        const dbName = data.database || '—';

        // Normalizar todas las tablas al formato interno: { name, columns: [{name,type,primary}] }
        const tables = rawTables.map(t => {
            if (Array.isArray(t.columns)) {
                // viejo formato → ya es array
                return { ...t, columns: t.columns.map(c => ({ ...c, type: c.type || 'string' })) };
            } else if (typeof t.columns === 'object' && t.columns !== null) {
                // nuevo formato → convertir objeto a array
                const cols = Object.entries(t.columns).map(([colName, colType]) => ({
                    name: colName,
                    type: colType || 'string',
                    primary: (t.primaryKeys || []).includes(colName)
                }));
            ...
            <div class="tl-wrapper">
                <div class="tl-toolbar">
                    <div class="tl-search-wrap">
                        ...
                        <input type="text" class="tl-search" id="tl-search-input" placeholder="Filtrar tablas y columnas..." value="${window.escapeHtml(this.searchTerm)}">
                    </div>
                </div>
                <div class="tl-db-name">${TableList.ICON_DB} <span id="tl-db-label">${window.escapeHtml(dbName)}</span></div>
                <div class="tl-tree" id="tl-tree">${this._renderTree(tables, this.searchTerm)}</div>
            </div>`;
        this._bindEvents();
    }

    _renderTree(tables, term) {
        if (!tables.length) return `<div class="tl-state">No se encontraron tablas</div>`;
        ...
        if (!filtered.length) return `<div class="tl-state">Sin resultados para "<b>${window.escapeHtml(term)}</b>"</div>`;

        let html = `<div class="tl-group-row open" data-group="tables"><span class="tl-chevron">▶</span><span>Tablas</span><span class="tl-col-count">${filtered.length}</span></div>`;
        ...
        filtered.forEach(table => {
            ...
            const colsHtml = (table.columns || []).map(col => {
                ...
                return `<div class="tl-col-row ${isPk ? 'is-pk' : ''}" data-col="${window.escapeHtml(table.name + '.' + col.name)}">
                    <span class="tl-col-icon">${icon}</span>
                    <span class="tl-col-name">${window.escapeHtml(col.name)}</span>
                    <span class="tl-col-type">${window.escapeHtml(typeShort)}</span></div>`;
            }).join('');
            ...
            html += `
                <div class="tl-table-row ${isOpen ? 'open' : ''}" data-table="${window.escapeHtml(table.name)}">
                    ...
                    <span class="tl-rel-badge" data-table="${window.escapeHtml(table.name)}"></span>
                </div>
                <div class="tl-col-list ${isOpen ? 'open' : ''}" data-cols-for="${window.escapeHtml(table.name)}">${colsHtml}</div>`;
        });
    ...
    _bindEvents() {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', () => {
                ...
                this._render();  // reutiliza la normalización interna
            });
        }
    ...
    _bindTreeEvents() {
        ...
        tree.querySelectorAll('.tl-table-row').forEach(row => {
            row.onclick = () => {
                ...
                // Cargar badge de relaciones (sin tocar SchemaExplorer)
        ...
        tree.querySelectorAll('.tl-col-row').forEach(row => {
            row.onclick = (e) => {
                e.stopPropagation();
                const fullName = row.dataset.col;
                if (window.app?.onColumnClick) app.onColumnClick(this.connectionId, fullName);
            };
        });
    }
    ...
    async loadTableDetails(tableName) {
        ...
        try {
            ...
            if (window.RapidBaseClient) {
                ...
                if (schema.success && schema.relations) {
                ...
                if (data.success && data.description[tableName]) {
                    relations = data.description[tableName].relations || [];
                }
            }
            ...
            if (badge) {
                ...
                badge.title = relations.map(r => `${r.type}: ${r.sourceTable}.${r.sourceColumn} → ${r.targetTable}.${r.targetColumn}`).join('\n');
            }
        } catch (e) {
